...
class ConfigModel extends Model
{
        ...
        if (is_array($config_list)) {
            foreach ($config_list as $k => $v) {
                ...
                if ($type == 'text') {
                    ...
                    $config_html .= '<div class="control-group"><label class="control-label">' . $v['info'] . '</label><div class="controls">';
                    $config_html .= '<textarea ' . $option . ' name="options[' . $v['name'] . ']" style="' . $style . '">' . $v['value'] . ' </textarea>';
                    $config_html .= '<img src="/Public/admin/images/help.gif" class="tips_img" data-title="' . $v['desc'] . '"></div></div>';
                } elseif ($type == 'radio') {
                    ...
                    foreach ($select_option as $select_k => $select_v) {
                        $option_one = explode(':', $select_v);
                        $config_html .= '<div>' . $option_one[1] . '</div> ' . '<input ' . $option . ' type="radio" value="' . $option_one[0] . '" name="options[' . $v['name'] . ']" ' . ($v['value'] == $option_one[0] ? 'checked' : '') . '> &nbsp;&nbsp;';
                    }
                    $config_html .= '<img src="/Public/admin/images/help.gif" class="tips_img" data-title="' . $v['desc'] . '"></div></div>';
                } elseif ($type == 'img') {
                    $config_html .= '<div class="control-group"><label class="control-label">' . $v['info'] . '</label><div class="controls">';
                    $config_html .= '<input id="' . $v['name'] . '" type="text" name="options[' . $v['name'] . ']" value="' . $v['value'] . '" style="' . $style . '">';
                    $config_html .= '<span id="img_' . $v['name'] . '"><img src="' . $v['value'] . '"></span><button class="btn btn-primary file" type="button">选择图片</button>';
                    $config_html .= '<img src="/Public/admin/images/help.gif" class="tips_img" data-title="' . $v['desc'] . '"></div></div>';
                }
            }
        }
     ...
     * @return mixed
     */
    static public function getDataByName($name)
    {
        $data = self::getDataByNameForS($name);
        if (empty($data)) {
            $data = self::getDataAll();
            S(self::fileName, $data, 3600 * 24);//存储缓存
            $data = self::getDataByNameForS($name);
        }
        return $data;
    }
     ...
     * @return mixed
     */
    static protected function getDataAll()
    {
        $where = array(
            'status' => 1,
        );
        $dataTemp = M('config')->where($where)->order('sort asc')->select();
        $data = array();
     ...
     * @return mixed
     */
    static private function getDataByNameForS($name)
    {
        $value = S(self::fileName);//读取缓存
